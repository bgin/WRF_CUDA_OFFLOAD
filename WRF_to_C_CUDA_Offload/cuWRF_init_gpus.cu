...
#if (LOG_ACTIVITY) == 1
	CuWRF_LOG("At prolog of devcaps_to_screen");
#endif
	cudaDeviceProp devp;
	cudaError status;
...
#if (CuWRF_DEBUG_ON) == 1
	_ASSERTE(dev_num >= 0 && NULL != fname);
#else
	_ASSERT(dev_num >= 0 && NULL != fname);
#endif

#if (LOG_ACTIVITY) == 1
	CuWRF_LOG("At prolog of devcaps_to_file");
#endif
	cudaDeviceProp devp;
	cudaError status;
#if (CuWRF_DEBUG_ON) == 1
	 CuWRF_DEBUG_CHECK(cudaGetDeviceProperties(&devp,dev_num));
#else
	 CuWRF_CHECK(cudaGetDeviceProperties(&devp,dev_num));
#endif
	// Check CUDA device no. x is installed
	if(devp.major == 9999 && devp.minor == 9999) {
		fprintf(stderr,"cudaGetDeviceProperties find out no CUDA device(s) ... terminating!!\n");
...
#define FREE_CPU_MEMORY               \
	 ...
	 _aligned_free(h_r4b);			\
	 _aligned_free(h_r4a);		    \
	 _aligned_free(h_i32c);		    \
	 _aligned_free(h_i32b);           \
	 _aligned_free(h_i32a);			
#endif
...
#endif
#if (LOG_ACTIVITY) == 1
	CuWRF_LOG("At prolog of gpu_vec_add_tests.\n");
#endif
	cudaError status;
	int32_t * __restrict dev_i32a = NULL;
	int32_t * __restrict dev_i32b = NULL;
	int32_t * __restrict dev_i32c = NULL;
	REAL4   * __restrict dev_r4a  = NULL;
	REAL4   * __restrict dev_r4b  = NULL;
	REAL4   * __restrict dev_r4c  = NULL;
	REAL8   * __restrict dev_r8a  = NULL;
	REAL8   * __restrict dev_r8b  = NULL;
	REAL8   * __restrict dev_r8c  = NULL;
	// Host arrays.
	int32_t * __restrict h_i32a = NULL;
	int32_t * __restrict h_i32b = NULL;
	int32_t * __restrict h_i32c = NULL;
	REAL4 *   __restrict h_r4a  = NULL;
	REAL4 *   __restrict h_r4b  = NULL;
	REAL4 *   __restrict h_r4c  = NULL;
	REAL8 *   __restrict h_r8a  = NULL;
	REAL8 *   __restrict h_r8b  = NULL;
	REAL8 *   __restrict h_r8c  = NULL;
	int32_t  ierr = 0;
